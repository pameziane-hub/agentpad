...
- Magnet cursor: terminals/TUIs expose NO accessibility click targets (one giant AXTextArea) — the magnet can never work there by design; native UI (menu bar, Dock, dialogs) is its domain. Menu bar targets come from the front app's AXMenuBar/AXExtrasMenuBar directly, NOT from point hit-tests (those return container/app roots up there). Other apps' status items stay invisible to it. All AX calls stay on the scanner's serial queue; CF refs are type-checked before casting (a buggy AX peer must not crash us).
- Probed AX latency (2026-06-11): warm hit-tests ~0.5 ms, first contact with a cold app 10–30 ms — never block the 120 Hz tick on AX.
